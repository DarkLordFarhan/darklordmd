...
// pendingPair stores { number, resolve, reject } while we wait for
// Baileys to call requestPairingCode at the right moment
...
async function startBot() {
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    // ── Key moment: Baileys is ready to accept requestPairingCode ──
    // This fires when the socket is in "connecting" state with a QR ready
    // We hijack it to request a pairing code instead
    if (qr && pendingPair) {
      const { number, resolve, reject } = pendingPair
      pendingPair = null
      try {
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    try {
      ...
      console.log(`📨 MSG type=${type} from=${m.key.remoteJid} keys=${Object.keys(m.message || {}).join(",")}`)

      const from = m.key.remoteJid
      const msg = m.message

      // Unwrap ephemeral/view-once/document-with-caption messages
      const actualMsg =
...
app.post("/api/pair", async (req, res) => {
  try {
    ...
    // If already connected, call requestPairingCode directly
    if (isConnected && sock) {
      try {
        const code = await sock.requestPairingCode(number)
        console.log(`📲 Direct pair — ${number}: ${code}`)
        ...
        return res.status(500).json({ success: false, error: "Code generation failed: " + err.message })
      }
    }

    // Not yet connected — restart bot and intercept the QR stage to inject pairing code
    console.log(`📲 Pair via QR intercept — ${number}`)
    ...
    // Create a promise that resolves when Baileys emits QR and we call requestPairingCode
    const code = await new Promise((resolve, reject) => {
      pendingPair = { number, resolve, reject }
      // Timeout after 30s
      setTimeout(() => {
        if (pendingPair) {
          pendingPair = null
