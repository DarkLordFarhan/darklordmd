...
async function startBot() {
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (qr && pairNumber && pairResolver) {
      console.log(`🔑 QR ready — requesting code for ${pairNumber}`)
      try {
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    for (const m of messages) {
      try {
        // Use Baileys built-in real message check
        if (!isRealMessage(m, sock.user?.id || "")) continue
        if (m.key.fromMe) continue
        if (m.key.remoteJid === "status@broadcast") continue

        const from = m.key.remoteJid

        // Extract the actual content regardless of nesting
...
app.post("/api/pair", async (req, res) => {
  try {
    ...
    // Already connected — call directly
    if (isConnected && sock) {
    ...
    // Not connected — restart fresh and intercept QR
    ...
    const result = await new Promise((resolve) => {
      ...
      setTimeout(() => {
        if (pairResolver) {
          pairResolver = null
          pairNumber = null
          resolve({ success: false, error: "Timed out. Please try again." })
        }
      }, 45000)
    })
