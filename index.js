...
// In-memory message cache for getMessage (needed for retries on linked devices)
...
async function startBot() {
  isConnected = false
  try {
    sock = await createSocket("./session")
  } catch (e) {
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (qr && pairNumber && pairResolver) {
      console.log(`🔑 QR ready — pairing ${pairNumber}`)
      try {
        const code = await sock.requestPairingCode(pairNumber)
        console.log(`✅ Code: ${code}`)
        const res = pairResolver
        pairResolver = null
        pairNumber = null
        res({ success: true, code })
      } catch (err) {
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const m of messages) {
      try {
        if (!m.message) { console.log("⚠️ No message content"); continue }
        if (m.key.fromMe) { console.log("⚠️ fromMe skip"); continue }
        if (m.key.remoteJid === "status@broadcast") continue

        const from = m.key.remoteJid

        // Cache message for retry handling
        msgCache.set(`${from}:${m.key.id}`, m.message)
        if (msgCache.size > 500) {
          const firstKey = msgCache.keys().next().value
          msgCache.delete(firstKey)
        }

        // Normalize to get real content (unwraps ephemeral, viewOnce etc)
        const content = normalizeMessageContent(m.message)
        if (!content) { console.log("⚠️ No normalized content"); continue }
        ...
        const logoPath = path.join(__dirname, "darklord-logo.png")
        ...
        if (cmd === ".menu") {
          if (fs.existsSync(logoPath)) {
            ...
            await sock.sendMessage(from, { text: "🔥 *DARKLORDMD V5.7*\n.ping .alive .owner .time .help" })
          }
        } else if (cmd === ".ping") {
          const start = Date.now()
          await sock.sendMessage(from, { text: `🏓 Pong! ${Date.now() - start}ms` })
        } else if (cmd === ".alive") {
          await sock.sendMessage(from, { text: "✅ *DarklordMD* is Online! 🔥" })
        } else if (cmd === ".owner") {
...
app.post("/api/pair", async (req, res) => {
  try {
    ...
    if (number.length < 7 || number.length > 15) return res.status(400).json({ success: false, error: "Invalid number." })

    if (isConnected && sock) {
      console.log(`📲 Direct pair for ${number}`)
      try {
        const code = await sock.requestPairingCode(number)
        sendWANotification(number, code)
    ...
    console.log(`📲 Fresh pair for ${number}`)
    if (sock) { try { sock.ws?.close() } catch (_) {} sock = null; isConnected = false }
    try { fs.rmSync("./session", { recursive: true, force: true }) } catch (_) {}
    ...
    const result = await new Promise((resolve) => {
      ...
      startBot()
      setTimeout(() => {
        if (pairResolver) { pairResolver = null; pairNumber = null; resolve({ success: false, error: "Timed out. Try again." }) }
      }, 45000)
    })

    if (result.success) sendWANotification(number, result.code)
...
function sendWANotification(number, code) {
  if (!sock) return
...
app.get("/darklord-logo.png", (req, res) => {
  const p = path.join(__dirname, "darklord-logo.png")
